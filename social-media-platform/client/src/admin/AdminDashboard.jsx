            ...
            <tbody>
              {users.map(u => (
                <tr key={u.id} className="border-t border-gray-100 dark:border-gray-700">
                  <td className="p-3"><Link to={`/profile/${u.username}`} className="flex items-center gap-2"><img src={avatarUrl(u.profile_picture)} alt="" className="w-8 h-8 rounded-full" /><div><p className="font-medium">{u.full_name}</p><p className="text-xs text-gray-500">@{u.username}</p></div></Link></td>
